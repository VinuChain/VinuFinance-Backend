...
import { keccak256, loadManifest, validateManifest } from "./reconcile-legacy.mjs";
...
assert.equal(usdtPools.filter((pool) => pool.declaredCollateralDecimals !== manifest.tokens[pool.collateralToken].decimals).length, 2);
const runtimeHashes = [
...
console.log(`reconcile-legacy self-test passed: ${manifest.pools.length} pools, ${usdtPools.filter((pool) => pool.declaredCollateralDecimals !== manifest.tokens[pool.collateralToken].decimals).length} explicit USDT decimal warnings`);
